...
public class AuthController : ControllerBase
{
    ...
    {
        _logger.LogInformation("🔐 Login-Versuch für Email: {Email}", request?.Email ?? "NULL");
        
        try
        {
            if (request == null)
            {
                _logger.LogWarning("⚠️ Login Request ist NULL");
                return BadRequest(new ApiResponse<AuthResponse>
                {
                    Success = false,
                    Message = "Ungültige Anfrage",
                    Errors = new[] { "Request body ist leer" }
                });
            }

            _logger.LogInformation("📞 Rufe AuthService.LoginAsync auf...");
            var result = await _authService.LoginAsync(request);
            
            if (result == null)
            {
                _logger.LogWarning("❌ Login fehlgeschlagen für {Email}", request.Email);
            ...
            _logger.LogInformation("✅ Login erfolgreich für {Email}", request.Email);
        ...
        {
            _logger.LogError(ex, "💥 EXCEPTION beim Login für {Email}: {Message}\nStackTrace: {StackTrace}", 
                request?.Email ?? "NULL", ex.Message, ex.StackTrace);
